...
// KDF constants are intentionally defined before any functions and use unique
// project-specific names to avoid Arduino macro/global name collisions.
enum : int {
  VAULT_KDF_ITERS_CURRENT = 120000,
  VAULT_KDF_ITERS_LEGACY  = 50000
};
...
bool deriveKey(const String& pin, const uint8_t* salt, uint8_t* out) {
  return deriveKeyWithIterations(pin, salt, VAULT_KDF_ITERS_CURRENT, out);
}
...
int tryUnlockFile(const char* path, const String& pin) {
  ...
  const int legacyKdfIterations = VAULT_KDF_ITERS_LEGACY;
  if (ret != 0 && legacyKdfIterations != VAULT_KDF_ITERS_CURRENT) {
    memset(key,0,32);
